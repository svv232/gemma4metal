...
int main() {
    ...
    auto run_layer = [&](const array& x, int l) -> array {
        ...
        if (cache_mode == 10 && global) {
            ...
            if (seq == 1 && q_cache[l].kq.value().shape(2) >= 999999) {
                // Direct scoring: Q @ K_quantized^T per KV head
                // q is (1, nh, 1, hd), K_quantized is (1, nkv, seq_cached, hd_packed)
                auto& kq = q_cache[l].kq.value();
                auto& ks_arr = q_cache[l].ks.value();
                auto& kb_arr = q_cache[l].kb.value();
                int cached_seq = kq.shape(2);

                // Compute scores per KV head group using quantized_matmul
                // quantized_matmul(x, w, scales, biases, transpose, group_size, bits)
                // x @ w^T where w is quantized: x=(1,hd), w=(cached_seq, hd_packed)
                int heads_per_kv = nh / nkv;
                std::vector<array> head_scores;
                for (int kh = 0; kh < nkv; kh++) {
                    // Extract this KV head's quantized data: (cached_seq, hd_packed)
                    auto kq_head = reshape(slice(kq, {0,kh,0,0}, {1,kh+1,cached_seq,kq.shape(3)}),
                                          {cached_seq, kq.shape(3)});
                    auto ks_head = reshape(slice(ks_arr, {0,kh,0,0}, {1,kh+1,cached_seq,ks_arr.shape(3)}),
                                          {cached_seq, ks_arr.shape(3)});
                    auto kb_head = reshape(slice(kb_arr, {0,kh,0,0}, {1,kh+1,cached_seq,kb_arr.shape(3)}),
                                          {cached_seq, kb_arr.shape(3)});
                    // For each Q head mapped to this KV head
                    for (int qh_off = 0; qh_off < heads_per_kv; qh_off++) {
                        int qh = kh * heads_per_kv + qh_off;
                        // q_head: (1, hd)
                        auto q_head = reshape(slice(q, {0,qh,0,0}, {1,qh+1,1,hd}), {1, hd});
                        // score: (1, cached_seq) = q_head @ kq_head^T
                        auto score = quantized_matmul(q_head, kq_head, ks_head,
                                     std::optional<array>(kb_head), true, 64, kb);
                        head_scores.push_back(score);
                    }
                }
                // Stack to (nh, 1, cached_seq) -> reshape to (1, nh, 1, cached_seq)
                auto scores = reshape(stack(head_scores, 0), {1, nh, 1, cached_seq});
                scores = scores * array(attn_scale);

                // Sliding window mask for non-global layers
                if (!global) {
                    int q_pos = cache_offset;  // position of current query
                    // Mask out tokens beyond sliding window
                    std::vector<float> mask_data(cached_seq, 0.0f);
                    for (int j = 0; j < cached_seq; j++) {
                        int k_pos = j;  // position of cached key
                        if ((q_pos - k_pos) >= sliding_window)
                            mask_data[j] = -1e9f;
                    }
                    auto mask = reshape(array(mask_data.data(), {cached_seq}, float32),
                                       {1, 1, 1, cached_seq});
                    scores = scores + mask;
                }
                ...
                // V still needs dequantization for weighted sum
                v_full = dequantize(q_cache[l].vq.value(), q_cache[l].vs.value(),
                                    q_cache[l].vb.value(), 64, vb);
                // attn = weights @ V: (1,nh,1,cached_seq) @ (1,nkv,cached_seq,hd)
                // Need to expand V for GQA
                if (nkv < nh) {
                    int repeats = nh / nkv;
                    // Tile V across head dimension
                    std::vector<array> v_expanded;
                    for (int kh = 0; kh < nkv; kh++)
                        for (int r = 0; r < repeats; r++)
                            v_expanded.push_back(slice(v_full, {0,kh,0,0}, {1,kh+1,cached_seq,hd}));
                    v_full = concatenate(v_expanded, 1);
                }
                auto attn_compressed = matmul(weights, v_full);
                // Skip the normal SDPA path below
                attn_compressed = reshape(transpose(attn_compressed, {0,2,1,3}), {1, seq, nh * hd});
                auto attn_out = qmm(attn_compressed, L+"self_attn.o_proj");
            ...
            // Fallback: full dequantize for short contexts or prefill
